...
def _validate_bancll_specification(bancll_name, bancll_specification_rows):
    ...
    if len(bancll_raw_table_names) > 1:
        ...
        raise exceptions.InvalidBANCLLSourceError(bancll_name, bancll_raw_table_names)
    ...
    if len(bancll_raw_column_names) > len(set(bancll_raw_column_names)):
        ...
        raise exceptions.DuplicateColumnError(bancll_name, bancll_raw_column_names_duplicated)
    ...
    if len(unknown_data_types) > 0:
        ...
        raise exceptions.UnknownDataTypeError(bancll_name, unknown_data_types)
    ...
    if min_initial_position < 0 or max_initial_position < 0:
        ...
        raise exceptions.NegativeColumnIndexError(bancll_name, negative_index)

    # [d.2] IS MIN_INDEX CORRECT ?
    if min_initial_position != 0:

        raise exceptions.InvalidMinColumnIndexError(bancll_name, min_initial_position)

    # [d.3] IS MAX_INDEX GREATER (OR EQUAL AT LEAST) THAN MIN_INDEX
    if max_initial_position < min_initial_position:

        raise exceptions.InvalidMaxColumnIndexError(bancll_name, max_initial_position, min_initial_position)
    ...
    if len(missing_positions) > 0:
        ...
        raise exceptions.NonContinuousRangeError(bancll_name, missing_positions)

    # [e] ARE ALL DATE_FORMATS CORRECTLY DEFINED ?
    undefined_formats: List[str] = list(filter(lambda x: x is not None,
                                               list(map(lambda x: JAVA_TO_PYTHON_FORMAT.get(x),
                                                        bancll_raw_column_input_formats))))

    if len(undefined_formats) > 1:

        raise exceptions.UndefinedDateFormatError(bancll_name, undefined_formats)
...
class BancllLoader(AbstractBancllLoader):
    ...
    def run(self, bancll_name: str, dt_business_date: str):
        ...
        self.__logger.info(f"Starting to validate specifications stated on such rows")
